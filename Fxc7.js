...
const { help, idiomas, donasi, limitend, limitcount, bottt } = require('./Fxc7/help')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:carlito\n'
            + 'ORG:Owner carlito;\n'
            + 'TEL;type=CELL;type=VOICE;waid=50769258167:+507 6925-8167\n'
            + 'END:VCARD'

prefix = "!"
name = "~ CARLITO BOT"
rregister = "GRACIAS POR REGISTRARTE PARA SER AMIGO DE ABBA BOT😁"
rmenu = "HOLA AMIGOS DE ABBADON BOT:)"
botinfo = "PARA INVITAR AL BOTS ES EL COMANDO !DUEÑO:)"
limitt = 50
memberLimit = 5
ban = []
premium = ["6581111451@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `HOLA @${num.split('@')[0]}\nBienvenido (A) al grupo de: *${mdata.subject}*\nEspero que la pases bien`
				...
				teks = `Adiós @${num.split('@')[0]} TE EXTRAÑAREMOS JAJAJA:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ espera está en curso no se caliente...*',
				success: '*éxito...*',
				error: {
					stick: ' *falló, se produjo un error al convertir la imagen en una pegatina*',
					Iv: '*lo siento enlace inválido!!*'
				},
				only: {
			                group: '*lo siento, este comando solo se puede usar en grupos!*',
					benned: '*lo siento, su número está prohibido, comuníquese con el propietario para abrir su prohibición*',
					ownerG: '*lo siento, este comando solo puede ser utilizado por el grupo propietario!*',
					ownerB: '*lo siento, este comando solo puede ser utilizado por el propietario del bot!* ',
					premium: '*lo siento, esta función es específicamente para usuarios premium!!*',
					userB: `Hola ${pushname2} No estás registrado en la base de datos. Escribe \n${prefix}register`,
					admin: '*lo siento, este comando solo puede ser utilizado por administradores de grupo!*',
					Badmin: '*lo siento, este comando solo se puede usar cuando el bot se convierte en administrador!*',
					publikG: `*lo siento, el bot ahora está privatizado por el propietario*\n*para más detalles escriba*\n*${prefix}infobot*`
				}
			}

			const botNumber = frhan.user.jid
			const ownerNumber = ["6581111451@s.whatsapp.net"] // owner number ubah aja
			...
			const checkLimit = (sender) => {
				...
				try {
					const getmemex = groupMembers.length
					    ...
					    frhan.sendMessage(from, `Perdón ${name} No se puede ingresar al grupo debido a miembros del grupo ${groupMetadata.subject} No cumple con el límite de miembros\n\nMiembro mínimo ${memberLimit}`, text)
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		if (isGroupAdmins) return reply(`${pushname2} Si el administrador de tu grupo no será expulsado`)
		...
		setTimeout( () => {
		reply(`Enlace detectado lo siento *${pushname2}* serás pateado spamero`)
		}, 0)
		}
		...
		case 'register':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('Ya eres amigo del bot de abbadon:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
					...
					captionnya = `╭─「 *REGISTRO DE USUARIO* 」\`\`\`\n│ El registro es exitoso \n│ BUENA ELECCION\`\`\`\n│\n│\`\`\`En ${date} ${time}\`\`\`\n│\`\`\`[Nombre]: ${pushname2}\`\`\`\n│\`\`\`[Numero]: wa.me/${sender.split("@")[0]}\`\`\`\n│\`\`\`Para usar el bot\`\`\`\n│\`\`\`Por favor\`\`\`\n│\`\`\`solo poner el comando ${prefix}menu\`\`\`\n│\`\`\`\n│Usuarios totales: ${user.length} Persona\`\`\`\n╰─────────────────────────`
					registerimg = await getBuffer(ppimg)
					frhan.sendMessage(from, registerimg, image, {quoted: mek, caption: captionnya})
					break 
			case 'help':
			case 'menu':
			if (isBanned) return reply(mess.only.benned)
			if (!isPublic) return reply(mess.only.publikG)
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.publikG)
				frhan.sendMessage(from, idiomas(prefix), text, {quoted: mek})
				break
				case 'donate':
				...
				case 'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nombre del bot* : ${me.name}\n➽ *Dueño del Bot* : @${ownerNumber}\n➽ *prefijo* : | ${prefix} |\n➽ *total bloqueado* : ${blocked.length}\n➽ *Activo Desde* : ${kyun(uptime)}\n\n➽ *Usuarios totales* : ${user.length} Usuario\n➽ *Instagram* : https://www.instagram.com/ssanchez.pty/\n➽ *Agradecimientos especiales a* : wa.me/50769258167`
				...
				case 'addprem':
					...
					premium = addpremium
					reply(`*Éxito agregado ${premium} Base de datos para usar Premium*`)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
					premium.splice(`${rprem}@s.whatsapp.net`, 1)
					reply(`Eliminar con éxito wa.me/${rprem} De usuario premium`)
					break
				case 'unban':
					if (!isOwner)return reply(mess.only.ownerB)
					bnnd = body.slice(8)
				...
				var found = false
                    ...
                    for (let lmt of limidat) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return reply(from,`Tu límite ha expirado`, id)
                            await reply(`*LIMITE SU ESTANCIA: ${limitCounts}*`)
                            found = true
                        }
                    }
                    if (found === false) {
				...
				case 'sticker':
						...
						reply(`Enviar fotos con subtítulos ${prefix}sticker o etiquetas de imagen que ya se han enviado`)
					}
					await limitAdd(sender) 
					break 
					case 'trigger':
            ...
            break  
		        ...
		        frhan.sendMessage(from, buffer, image, {quoted: mek, caption: `*Nombre* : ${groupName}\n*Miembros* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*Descripciòn* : ${groupDesc}`})
                break
				case 'trendtwit':
					frhan.updatePresence(from, Presence.composing) 
                     if (!isUser) return reply(mess.only.userB)
               ...
               frhan.updatePresence(from, Presence.composing) 
              options = {
          text: `El propietario del grupo es : wa.me/${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           frhan.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'leave': 
				    if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			    	anu = await frhan.groupLeave(from, `Adiós a todos los miembros *${groupMetadata.subject}*`, groupId)
	                break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return frhan.sendMessage(from, '¿Qué código de idioma?\n Si no conoce el código de idioma, escríba *${prefix}idiomas*', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return frhan.sendMessage(from, '¿Dónde está el texto??', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('El texto es demasiado')
                ...
                frhan.sendMessage(from, `Velocidad: ${latensi.toFixed(4)} _Secundo_`, text, { quoted: mek})
                    break
                case 'tagme':
                if (isBanned) return reply(mess.only.benned)
                if (!isPublic) return reply(mess.only.publikG)
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('50769258167@s.whatsapp.net', options, text, {quoted: mek})
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('50769258167@s.whatsapp.net', options, text, {quoted: mek})
                    reply('PETICIÓN USTED HA LLEGADO AL DUEÑO DEL BOT, Peticiones manu/main2 no será respondido.')
                    break
				case 'memeindo':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    reply(mess.wait)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`\`\`\`✓Grupo de apertura con éxito\`\`\` *${groupMetadata.subject}*`)
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido, emitido : @${mentioned[0].split('@')[0]}`, mentioned, true)
						frhan.groupRemove(from, mentioned)
					}
					break 
				case 'kicktime':
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Orden recibida, @${mentioned[0].split('@')[0]} Te conviertes en administrador del grupo *${groupMetadata.subject}*`, mentioned, true)
						frhan.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recibidos, no eres administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Orden recibida : @${mentioned[0].split('@')[0]} Hágase miembro`, mentioned, true)
						frhan.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'bott':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Seleccione habilitar o deshabilitar!')
					if (args[0] === 'on') {
						if (isPublic) return reply('Activado')
						publik.push(from)
						fs.writeFileSync('./database/json/public.json', JSON.stringify(publik))
						reply(`Éxito por favor escriba ${prefix}menu Para los comandos del bot`)
					} else if (args[0] === 'off') {
						publik.splice(from, 1)
						fs.writeFileSync('./database/json/public.json', JSON.stringify(publik))
						reply(`Los miembros del grupo no pueden usar el bots`)
					} else {
						reply('Seleccionar encendido / apagado')
					}
					break
				case 'simih':
				if (isBanned) return reply(mess.only.benned)    
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Activódo con éxito la función de bienvenida en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply(`\`\`\`✓Desactivado con éxito la función de bienvenida en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('escriba! antienlace para activar')
					if ((args[0]) === 'on') {
						if (isAntiLink) return reply('ta activo')
						antilink.push(from)
...
case 'asupan':
		...
		if (isLimit(sender)) return reply(limitend(pushname2))
		 ...
		 case 'tomp3':
				...
				if (!isPublic) return reply(mess.only.publikG)
                	frhan.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('_*Responde el video!*_')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await frhan.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Lo siento, Comando *${prefix}${command}* No registrado en la base de datos *${prefix}menu*`)
                  }
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ABBADONBOT]','aqua'), 'Comando no registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
